...
class Cluster {
     ...
     * @memberof Cluster
     */
    async connect(firstShardID, lastShardID, maxShards, token, type, clientOptions) {
        ...
        const bot = new this.class(token, options);
        this.bot = bot;
    ...
    async loadCode(bot) {
        ...
        let path = join(rootPath, this.mainFile);
        let { default: app } = await import('file:///' + path)?.default || await import('file:///' + path);
