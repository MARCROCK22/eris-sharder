...
class ClusterManager extends EventEmitter {
     ...
     */
    constructor(token, mainFile, options, CLASS = Eris.Client) {
        super();
        this.CLASS = CLASS
        ...
        if (this.token) {
            this.eris = new CLASS(token, options);
            this.launch(false);
        } else {
    ...
    launch(test) {
        if (master.isMaster) {
            ...
            const Cluster = new cluster(this.CLASS);
            Cluster.spawn();
        }
        ...
        this.queue.on("execute", item => {
            ...
            if (cluster) {
                setTimeout(() => master.workers[cluster.workerID].send(item.value), 500)
            }
        });
    }
